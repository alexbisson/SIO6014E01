...
class DrawingManager {
  ...
  ImageElement _planeRight;
  ImageElement _planeTop;
  ImageElement _planeLeft;
  ImageElement _planeBottom;

  DrawingManager(){
    _canvas = document.query('#canvasMap');
    _planeRight = document.query('#plane-right');
    _planeTop = document.query('#plane-top');
    _planeLeft = document.query('#plane-left');
    _planeBottom = document.query('#plane-bottom');
  ...
  void drawPlane(int x, int y, int orientation) {
    if (orientation >= -45 && orientation < 45) {
      _context.drawImage(_planeRight, x - 16, y - 16, 32, 32);
    } else if (orientation >= 45 && orientation < 135) {
      _context.drawImage(_planeTop, x - 16, y - 16, 32, 32);
    } else if (orientation >= 135 && orientation < 225) {
      _context.drawImage(_planeLeft, x - 16, y - 16, 32, 32);
    } else {
      _context.drawImage(_planeBottom, x - 16, y - 16, 32, 32);
    }
  }
